...
contract bank {
    ...
    event DepositWithdrawError(string message);

    modifier CheckMinDepAmt(uint64 amt) {
        if (amt > MinDepAmt) {
            status = Status.Successful;
            emit DepositWithdrawError("Deposit Succesfull");
        } else {
            status = Status.Failed;
            emit DepositWithdrawError("Deposit Failed");
        }
    ...
    modifier CheckMaxWithdAmt(uint64 amt) {
        if (amt < MaxWithdAmt) {
            status = Status.Successful;
            emit DepositWithdrawError("Withdrawal Successful");
        } else {
            status = Status.Failed;
            emit DepositWithdrawError("Withdrawal Failed");
    ...
    modifier VerifyUname(string memory uname) {
        require(users[uname].uaddress != address(0), "User not Found!");
        _;
    }
